...
function userCheck($username, $user_email) {
    ...
    if (eregi("^((root)|(adm)|(linux)|(webmaster)|(admin)|(god)|(administrator)|(administrador)|(nobody)|(anonymous)|(anonimo)|(an�nimo)|(operator))$",$username)) $stop = "<center>"._NAMERESERVED."";
...
function confirmNewUser($username, $user_email, $user_password, $user_password2, $random_num, $gfx_check) {
    global $stop, $EditedMessage, $sitename, $module_name, $minpass, $currentlang;
    include("header.php");
    include("config.php");
...
function finishNewUser($username, $user_email, $user_password, $random_num, $gfx_check) {
	...
	$db->sql_query("INSERT INTO ".$user_prefix."_pendingusers (user_id, username, user_email, user_password, user_regdate, check_num, time) VALUES (NULL, '$username', '$user_email', '$user_password', '$user_regdate', '$check_num', '')");
	if(!$result) {
	    echo ""._ERROR."<br>";
	 #  Added lines to add users into chat database.
...
function userinfo($username, $bypass=0, $hid=0, $url=0) {
    ...
    CloseTable();
    #
    # Here's where henry is going to put the logic to show the pick percentages in
    # each pool....
    # 
    if (is_user($user)) {
    	cookiedecode($user);
    ...
    OpenTable();
    echo "<!-- Start of Pool participation -->\n";
    echo "<center><b>Pool Participation</b><br><br>";
    ...
    foreach ($poollist as $pool) {
    	...
    	# check membership;
 			...
 			$leagueID=$poolname='';
 			$DST_end=$DST_start=$ppercent=$testing=$top25=0;
 			if ($debug > 1) { echo "\n<!-- HBT Reading in \"modules/$pool/config.inc.php\" -->\n"; }
 			require("modules/$pool/config.inc.php");
    	foreach ($membership as $sub) {
    		# echo "<!-- \$sub=\"$sub\", \$poolname=\"$poolname\", \$prefix=\"$prefix\", \$leagueID=\"$leagueID\",\$today_date=\"$today_date\", \$now_time=\"$now_time\", -->\n"; 
    		if (eregi( $sub, $poolname )) {
    			# echo "<!-- MATCH!  \$sub=\"$sub\", \$poolname=\"$poolname\" -->\n";
    			# $subscribed[$pool] =1;
    			# echo "$pool<br>\n";
    			#http://football-pools.org/modules.php?name=Pool-NCAA-26NOS&op=MakePicks&seasonID=2013
    			echo "<tr><td><b><a href=\"/modules.php?name=$pool&op=MakePicks&seasonID=$seasonID\">$pool ($ppercent)</a></b><br></td><td>";
    			...
    			if(is_object($object)) {
    				...
    				echo "The next game is $date at $time<br>\n";
    			} else {
    			...
    			if ($debug > 0) { echo "<!-- HBT4 \$seasonID=\"$seasonID\", \$leagueID=\"$leagueID\", \$weekID=\"$weekID\" -->\n"; }
    			...
    			if ($debug > 0) { echo "<!-- HBT-PickPct \$sql='$sql' -->\n"; }	
    			while ($row = $db->sql_fetchrow($result)) {
    				if ($row['week'] < $weekID) {
    					if ($debug > 2) { echo "<!-- HBT-PickPct2 pastgames row !! -->\n"; }
    					if ($usespreads == 1) {
    					...
    					if ($debug > 2) { echo "<!-- HBT-PickPct2 thisweekgames row !! -->\n"; }
    					if ($usespreads == 1) {
    					...
    					if ($debug > 1) { echo "<!-- HBT-PickPct2 futuregames row !! -->\n"; }
    					$futuregames++;
    				}
    			}
    			...
    			if ($debug > 0) { echo "<!-- HBT6 user's picked games \$sql='$sql' -->\n"; }
    			$result = $db->sql_query($sql, $dbi);
    			while ($row = $db->sql_fetchrow($result)) {
    				if ($debug > 3) { echo "<!-- HBT6 row of picks  -->\n"; }
    				if ($row['week'] < $weekID) {
    					$pastpicked++;
    					if ($debug > 1) { $week= $row['week']; $game= $row['game']; echo "<!-- HBT6 \$week=\"$week\", \$game=\"$game\", \$pastpicked='$pastpicked' -->\n"; }
    				} elseif ($row['week'] == $weekID) {
    					$thisweekpicked++;
    					if ($debug > 1) { $week= $row['week']; $game= $row['game']; echo "<!-- HBT6 \$week=\"$week\", \$game=\"$game\", \$thisweekpicked='$thisweekpicked' -->\n"; }
    				}
    			}
    			# figure past, present, and future (needed) percentages.
    			if ($debug > 0) { echo "<!-- HBT \$weekID=\"$weekID\",\$pastgames=\"$pastgames\",\$pastpicked=\"$pastpicked\",\$thisweekgames=\"$thisweekgames\",\$thisweekpicked=\"$thisweekpicked\",\$futuregames=\"$futuregames\" -->\n"; }
    			$past_pct=(($pastpicked/$pastgames)*100);
    			if ($debug > 1) { echo "<!-- \$past_pct=((\$pastpicked/\$pastgames)*100) -->\n"; }
    			if ($debug > 1) { echo "<!-- $past_pct=(($pastpicked/$pastgames)*100) -->\n"; }
    			$thisweek_pct=number_format(($thisweekpicked/$thisweekgames)*100,1);
    			if ($debug > 1) { echo "<!-- \$thisweek_pct=number_format((\$thisweekpicked/\$thisweekgames)*100,1) -->\n"; }
    			if ($debug > 1) { echo "<!-- $thisweek_pct=number_format(($thisweekpicked/$thisweekgames)*100,1) -->\n"; }
    			$allpicked=$thisweekpicked+$pastpicked;
    			if ($debug > 1) { echo "<!-- \$allpicked=\$thisweekpicked+\$pastpicked -->\n"; }
    			if ($debug > 1) { echo "<!-- $allpicked=$thisweekpicked+$pastpicked -->\n"; }
    			$allgames=$pastgames+$thisweekgames+$futuregames;
    			if ($debug > 1) { echo "<!-- \$allgames=\$pastgames+\$thisweekgames+\$futuregames -->\n"; }
    			if ($debug > 1) { echo "<!-- $allgames=$pastgames+$thisweekgames+$futuregames -->\n"; }
    			if ($weekID > 1) { $past_pct=number_format(($pastpicked/$pastgames)*100,1); }
    			if ($debug > 1) { echo "<!-- \$past_pct=number_format((\$pastpicked/\$pastgames)*100,1) -->\n"; }
    			if ($debug > 1) { echo "<!-- $past_pct=number_format(($pastpicked/$pastgames)*100,1) -->\n"; }
    			$futurepicks=number_format((($ppercent/100)*$allgames)-$allpicked,0);
    			if ($debug > 1) { echo "<!-- \$futurepicks=number_format(((\$ppercent/100)*\$allgames)-\$allpicked,0) -->\n"; }
    			if ($debug > 1) { echo "<!-- $futurepicks=number_format((($ppercent/100)*$allgames)-$allpicked,0) -->\n"; }
    			$future_pct=number_format(($futurepicks/$futuregames)*100,1);
    			if ($debug > 1) { echo "<!-- \$future_pct=number_format((\$futurepicks/\$futuregames)*100,1) -->\n"; }
    			if ($debug > 1) { echo "<!-- $future_pct=number_format(($futurepicks/$futuregames)*100,1) -->\n"; }
    			...
    			echo "<b>You've picked $hl_on$thisweek_pct%$hl_off of the games this week";
    			if ($weekID > 1) { echo " and $past_pct% of the prior games"; }
    			echo ".</b><br>\n";
    			if ($leagueID == 'NCAA' && $top25 == '1') {
    				...
    				if ($debug > 1) { 	echo "<!-- \$low_allgames = \$pastgames+\$thisweekgames+\$low_gr; -->\n"; }
    				if ($debug > 1) { 	echo "<!-- $low_allgames = $pastgames+$thisweekgames+$low_gr; -->\n"; }
    				$high_allgames = $pastgames+$thisweekgames+$high_gr;
    				if ($debug > 1) { 	echo "<!-- \$high_allgames = \$pastgames+\$thisweekgames+\$high_gr; -->\n"; }
    				if ($debug > 1) { 	echo "<!-- $high_allgames = $pastgames+$thisweekgames+$high_gr; -->\n"; }
    				$low_pgr = number_format((($ppercent/100)*$low_allgames)-$allpicked,0);
    				if ($debug > 1) { 	echo "<!-- \$low_pgr = number_format((($ppercent/100)*\$low_allgames)-\$allpicked,0); -->\n"; }
    				if ($debug > 1) { 	echo "<!-- $low_pgr = number_format((($ppercent/100)*$low_allgames)-$allpicked,0); -->\n"; }
    				$high_pgr = number_format((($ppercent/100)*$high_allgames)-$allpicked,0);
    				if ($debug > 1) { 	echo "<!-- \$high_pgr = number_format((($ppercent/100)*\$high_allgames)-\$allpicked,0); -->\n"; }
    				if ($debug > 1) { 	echo "<!-- $high_pgr = number_format((($ppercent/100)*$high_allgames)-$allpicked,0); -->\n"; }
    				$low_f_pct = number_format(($low_pgr/$low_gr)*100,1);
    				$high_f_pct = number_format(($high_pgr/$high_gr)*100,1);
    				echo "You can count on between $low_gr and $high_gr games to pick for the rest of the season;<br>";
    				if ((($low_f_pct >= 100)&&($high_f_pct <= 100)) && (($past_pct < $ppercent) && ($thisweek_pct < $ppercent))) {
    					if ($debug > 0) { echo "<!-- ($low_pgr, $high_pgr)) -->\n"; }
    					echo " so you'll have to pick at least $high_pgr ($high_f_pct%) more games;";
    					echo " You might not be able to pick enough games ";
    				} elseif (($low_pgr <= 0) && (($past_pct < $ppercent) && ($thisweek_pct < $ppercent))) {
    					echo " so you'll have to pick between $low_pgr ($low_f_pct%) and $high_pgr ($high_f_pct%) more games";
    				} else {
    					echo " so you'll have to pick between $low_pgr ($low_f_pct%) and $high_pgr ($high_f_pct%) more games";
    				}
    				echo " to be in the end-of-season stats.";
    			} else {
    				if ($future_pct < 0) {
    					echo "You don't have to pick any more games";
    				} elseif ($future_pct > 100) {
    					echo "Sorry, you won't be able to pick enough games for the rest of the year";
    				} elseif ($future_pct >= 95) {
    					echo "<font color=darkred>BE CAREFULL ! &nbsp;You have very little 'wiggle room'! You must pick $future_pct% of all remaining games";
    				} else {
    					echo "You have to make at least $futurepicks more picks this year, or $future_pct% of the remaining games ";
    				}
    				echo " to be in the End-of-season stats.<br>\n";
    				if ($usespreads == 1) {
    					echo "Keep in mind that some of the remaining games might NOT have spreads, so the percentage you have to pick might actually be higher or lower.<br>";
    				}
    				if ($future_pct >= 95) { echo "</font>"; }
    			}
    			...
    			# echo "<!-- NO MATCH !! \$sub=\"$sub\", \$poolname=\"$poolname\" -->\n";
    		}
    	}
    }
    echo "</tr></table>\n";
    # echo "<!-- \$userinfo[member_of]=\"$userinfo[member_of]\"-->\n";
    $sql_p = "SELECT ";
    CloseTable();
    #
...
function new_user() {
    global $my_headlines, $module_name, $db, $gfx_chk, $currentlang;
    if (!is_user($user)) {
	mt_srand ((double)microtime()*1000000);
	$maxran = 1000000;
	$random_num = mt_rand(0, $maxran);
