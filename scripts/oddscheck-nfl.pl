...
$oddsurl = "http://www.vegasinsider.com/nfl/odds/las-vegas/";
...
$sql .= " WHERE date >= '$bdate' AND league = 'NFL' AND season = '$seasonID'";
...
$sql .= " WHERE season <= '$seasonID' AND league = 'NFL'"; 
$sql .= " GROUP BY team_id ) latestteam";

$cursor =$dbh->prepare($sql) || die "Cannot prepare statement: $DBI::errstr\n";
$cursor->execute;
while(@fields = $cursor->fetchrow) {
	if ($fields[1] =~ m/NY/ ) { $fields[1] =~ s/NY/N.Y./; }
	if ($fields[1] =~ m/LA/ ) { $fields[1] =~ s/LA/L.A./; }
...
$sql.=" WHERE (league='NFL' AND season = '$seasonID' AND week = '$week')";
...
$start = "<tr><td class='viHeaderNorm'>NFL SPREAD</td></tr>";
$begin_row = '<span class="cellTextHot">';
# in the array, the x+1 is visitor, x+2 is home.
#$team_regexp = '<span class="team"><a href="http://sports.yahoo.com/nfl/teams/[^>]+>([^<]+)</a>';
#$team_regexp = '\d+&nbsp;<b><a href="/nfl/teams/team-page.cfm/team/([A-Za-z]+)" class="tabletext" title="([[:space:]A-Za-z\.-]+)">[[:space:]A-Za-z\.-]+</a></b>';
$team_regexp = '\d+&nbsp;<b><a href="/nfl/teams/team-page\.cfm/team/([A-Za-z]+)" title="([[:space:]A-Za-z\.-]+) Team Schedule and ATS Results" class="tabletext">([^<]+)</a></b>';
...
while(@fields = $cursor->fetchrow) {
	...
	print("'$dispteamname{$fields[1]}' @ '$dispteamname{$fields[0]}', game #$fields[2] needs a spread.\n");
}
	...
	for ($x=0; $x <= $#s_spread; $x++) {
		if (($s_home[$x] eq $hometeam) && ($s_visitor[$x] eq $visitingteam)) {
			...
			if (!($s_spread[$x] == 0)) {
				...
				$sql .= " WHERE league = 'NFL' AND season = '$seasonID' AND week = '$week' AND game = '$game'";
				$sql .= " AND `home` = '$hometeam' AND `visitor` = '$visitingteam' AND `home_spread` is NULL LIMIT 1";
				print ("\"$sql\"\n");
...
if ($debug > 0 || $updates > 0) {
	...
	print SENDMAIL "Subject: Football Pools (NFL) - $updates spreads $verb made\n\n";
