...
# For testing !!
#$week=9;
#if ($debug > 1) { print ("\$week set to='$week' for testing\n"); }
#
...
$scoreurl="https://www.vegasinsider.com/college-football/scoreboard/scores.cfm/week/$week/season/$season";
if ($week == '15') {
  $scoreurl='https://www.vegasinsider.com/college-football/scoreboard/'; 
}
...
#my $content = `curl -k -o -- $scoreurl`;
#my $content = "/usr/bin/curl -q -o- $scoreurl";
my $content = read_file("scores.html");
...
for ($y=0; $y < $#scores+1; $y++) {
	...
	if ( $scores[$y] =~ /$gamedate/ ) {
		...
		if ($debug > 0) { print ("Matched date: \$dbdate = \"$dbdate\", \$gweek = \"$gweek\"\n"); }
	}
	# Match the "gameregexp" line....   regexp has to be the LAST check, for capture.
		...
		if ($vteamid > 0 && $hteamid > 0 ) {
			print ("Updating $teamname{$vteamid} $vteamscore vs. $teamname{$hteamid} $hteamscore\n");
			if ($debug > 1) { print ("\$vteamid=\"$vteamid\"; \$vteamscore=\"$vteamscore\"; \$hteamid=\"$hteamid\"; \$hteamscore=\"$hteamscore\"\n") }
			...
			print ("Don\'t recognize team $vteamid ($teamname{$vteamid}) or $hteamid ($teamname{$hteamid})\n");
		}
	}
}
